    ...
    class npsoParticleClusterAndOutliers():
        ...
        def updateVelocityandPosition(self,bestNeighborState,globalBestState,t,constrict=None): 
            ...
            if self.Kp > np.shape(self.centroids)[0]: 
                outputSplitCentroids = self.splitCentroids(self.Kp,self.weights,self.centroids,self.velCentroids,
                                                           self.data,self.assignData,self.cntEmptyClusters);                
                ...
                outputDeleteCentroids = self.deleteCentroids(self.Kp,self.weights,self.centroids,self.velCentroids,
                                                             self.assignData,self.cntEmptyClusters,self.data);
        ...
        def updateLocalBest(self):
            ...
            localBestState = (self.localBestFitness,self.localBestKp,self.localBestCentroid,
                              self.localBestNumEmptyClusters,self.localBestl,self.localBestOutliers,self.localBestWeights,self.localBestWeightsOutliers)
            ...
            (self.localBestFitness,self.localBestKp,self.localBestCentroid,self.localBestNumEmptyClusters,
             self.localBestl,self.localBestOutliers,self.localBestWeights,self.localBestWeightsOutliers) = localBestState
