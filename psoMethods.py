    ...
    def executeNPSO(self,neighborSize,w,posMin,posMax,velMin,velMax,numIters,numParticles,particle,optimType,numEvalState,fitnessFunction,fitnessEvaluationFunction,npsoInterpFunc):
        ...
        for t in range(numIters): 
            for j in range(numParticles):  
                ...
                globalBestState = npsoInterpFunc(optimType,JState,KState,posMin,posMax,globalBestState,swarm[j].data,swarm[j].distFunc,swarm[j].splitCentroids,
                                      swarm[j].deleteCentroids,swarm[j].assignData,swarm[j].calcCentroids,swarm[j].cntEmptyClusters,swarm[j].findOutliers,swarm[j].removeOutliersFromClusters,fitnessFunction,fitnessEvaluationFunction)
